...
getDiagnosticContext()
  ...
  .catch(async (err) => {
    await error(`Astro Studio startup logging failed: ${err}`);
  });

// Global error handlers for JavaScript errors
window.addEventListener('error', (event) => {
  void error(
    `Astro Studio [JS_ERROR] ${(event.error as Error)?.message || event.message} at ${event.filename}:${event.lineno}`,
  );
});

window.addEventListener('unhandledrejection', (event) => {
  void error(`Astro Studio [PROMISE_REJECTION] ${String(event.reason)}`);
});
